...
const index = require("./index-Dwm0THD7.cjs");
...
function createLocation(state, node, location2) {
  ...
  if (node.type === "element") {
    ...
    if (state.verbose) {
      ...
      _default.ok(location2.startTag);
...
function fromHtml(value, options) {
  ...
  const file = value instanceof index.VFile ? value : new index.VFile(value);
  ...
  function internalOnerror(error) {
    ...
    if (level) {
      ...
      const message = new index.VFileMessage(format(info.reason), {
        place: {
          start: {
