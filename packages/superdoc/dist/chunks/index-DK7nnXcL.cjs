...
const ignorableNode = index.convert(["comment", "doctype"]);
...
const findAfter = (
   ...
   */
  (function(parent, index$1, test) {
    const is = index.convert(test);
    if (!parent || !parent.type || !parent.children) {
      throw new Error("Expected parent node");
    }
    if (typeof index$1 === "number") {
      if (index$1 < 0 || index$1 === Number.POSITIVE_INFINITY) {
    ...
    while (++index$1 < parent.children.length) {
      if (is(parent.children[index$1], index$1, parent)) {
  ...
  if (properties.type === "email" || properties.type === "url") {
    ...
    while (++index3 < values.length) {
      ...
      const result2 = {
        ...
        children: [{ type: "text", value: values[index3][1] || value2 }]
      };
...
function phrasing(node) {
  ...
  return tagName ? phrasing$1({ type: "element", tagName, properties: {}, children: [] }) : index.phrasing(node);
}
...
function media(state, node) {
  ...
  index.visit(fragment, function(node2) {
    if (node2.type === "link") {
  ...
  while (!source && ++index$1 < node.children.length) {
...
function inspect(node) {
  ...
  let rowIndex = 0;
  let cellIndex = 0;
  index.visit(node, function(child) {
    if (child.type === "element") {
...
function toMdast(tree, options) {
  ...
  index.visit(cleanTree, function(node) {
  ...
  index.visit(mdast, function(node, index2, parent) {
    if (node.type === "text" && index2 !== void 0 && parent) {
