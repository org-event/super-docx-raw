...
require("./SuperConverter--V0Gvst5.cjs");
...
const errorUnexpectedEndOfArray = index.create("Unexpected end of array");
const errorIntegerOutOfRange = index.create("Integer out of Range");
class Decoder {
  /**
...
const readVarUint = (decoder) => {
  ...
  while (decoder.pos < len) {
    ...
    num = num + (r & index.BITS7) * mult;
    mult *= 128;
    if (r < index.BIT8) {
      return num;
    }
    if (num > index.MAX_SAFE_INTEGER) {
...
const _readVarStringNative = (decoder) => (
  /** @type any */
  index.utf8TextDecoder.decode(readVarUint8Array(decoder))
);
const readVarString = index.utf8TextDecoder ? _readVarStringNative : _readVarStringPolyfill;
const getUnixTime = Date.now;
class ObservableV2 {
  constructor() {
    this._observers = index.create$1();
  }
  /**
   ...
   */
  emit(name, args) {
    return index.from((this._observers.get(name) || index.create$1()).values()).forEach((f) => f(...args));
  }
  destroy() {
    this._observers = index.create$1();
  }
...
class Observable {
   ...
   */
  on(name, f) {
    index.setIfUndefined(this._observers, name, index.create$2).add(f);
  }
  /**
   * @param {N} name
   * @param {function} f
...
class Awareness extends Observable {
  ...
  constructor(doc) {
    ...
    }, index.floor(outdatedTimeout / 10));
    doc.on("destroy", () => {
      this.destroy();
    });
    this.setLocalState({});
  ...
  setLocalState(state) {
    ...
    if (state === null) {
      ...
      if (!index.equalityDeep(prevState, state)) {
...
const encodeAwarenessUpdate = (awareness, clients, states = awareness.states) => {
  ...
  const encoder = index.createEncoder();
  index.writeVarUint(encoder, len);
  for (let i = 0; i < len; i++) {
    const clientID = clients[i];
    ...
    index.writeVarUint(encoder, clientID);
    index.writeVarUint(encoder, clock);
    index.writeVarString(encoder, JSON.stringify(state));
  }
  return index.toUint8Array(encoder);
};
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (currClock < clock || currClock === clock && state === null && awareness.states.has(clientID)) {
      ...
      if (clientMeta === void 0 && state !== null) {
        ...
        if (!index.equalityDeep(state, prevState)) {
          filteredUpdated.push(clientID);
        }
        updated.push(clientID);
      }
...
const formItemInjectionKey = index.createInjectionKey("n-form-item");
...
function useStyle(mountId, style2, clsPrefixRef) {
  if (!style2) {
    if (vue.process$1.env.NODE_ENV !== "production") index.throwError("use-style", "No style is specified.");
    return;
  }
  const ssrAdapter = index.useSsrAdapter();
  const NConfigProvider = vue.inject(index.configProviderInjectionKey, null);
  const mountStyle = () => {
    const clsPrefix = clsPrefixRef.value;
    style2.mount({
      id: clsPrefix === void 0 ? mountId : clsPrefix + mountId,
      head: true,
      anchorMetaName: index.cssrAnchorMetaName,
      props: {
...
const style$4 = index.cB("base-icon", `
 height: 1em;
 width: 1em;
 line-height: 1em;
 text-align: center;
...
const NIconSwitchTransition = vue.defineComponent({
  ...
  setup(_, {
    ...
    const isMountedRef = index.isMounted();
...
function replaceable(name, icon) {
  ...
  return vue.defineComponent({
    ...
    setup() {
      var _a;
      const mergedIconsRef = (_a = vue.inject(index.configProviderInjectionKey, null)) === null || _a === void 0 ? void 0 : _a.mergedIconsRef;
      return () => {
        var _a2;
...
function iconSwitchTransition({
  ...
  return [index.c("&.icon-switch-transition-enter-from, &.icon-switch-transition-leave-to", {
    transform: `${originalTransform} scale(0.75)`,
    left,
    top,
    opacity: 0
  }), index.c("&.icon-switch-transition-enter-to, &.icon-switch-transition-leave-from", {
    transform: `scale(1) ${originalTransform}`,
    left,
    top,
    opacity: 1
  }), index.c("&.icon-switch-transition-enter-active, &.icon-switch-transition-leave-active", {
...
const style$3 = index.cB("base-close", `
 display: flex;
 align-items: center;
...
`, [index.cM("absolute", `
 ...
 `), index.cNotM("disabled", [index.c("&:hover", `
 color: var(--n-close-icon-color-hover);
 `), index.c("&:hover::before", `
 background-color: var(--n-close-color-hover);
 `), index.c("&:focus::before", `
 background-color: var(--n-close-color-hover);
 `), index.c("&:active", `
 color: var(--n-close-icon-color-pressed);
 `), index.c("&:active::before", `
 background-color: var(--n-close-color-pressed);
 `)]), index.cM("disabled", `
 cursor: not-allowed;
 color: var(--n-close-icon-color-disabled);
 background-color: transparent;
 `), index.cM("round", [index.c("&::before", `
 border-radius: 50%;
 `)])]);
const NBaseClose = vue.defineComponent({
  ...
  setup(props) {
    useStyle("-base-close", style$3, vue.toRef(props, "clsPrefix"));
    return () => {
      const {
        clsPrefix,
        disabled,
...
const style$2 = index.c([index.c("@keyframes rotator", `
 0% {
 ...
 }`), index.cB("base-loading", `
 position: relative;
 line-height: 0;
 width: 1em;
 height: 1em;
 `, [index.cE("transition-wrapper", `
 position: absolute;
 width: 100%;
 height: 100%;
 `, [iconSwitchTransition()]), index.cE("placeholder", `
 position: absolute;
 left: 50%;
 top: 50%;
...
const NBaseLoading = vue.defineComponent({
  ...
  setup(props) {
    useStyle("-base-loading", style$2, vue.toRef(props, "clsPrefix"));
  },
  render() {
    const {
      clsPrefix,
...
function fadeInHeightExpandTransition({
  ...
  return [index.c(`&.fade-in-height-expand-transition-${leaveClass}-from,
 &.fade-in-height-expand-transition-${enterClass}-to`, Object.assign(Object.assign({}, enterToProps), {
    opacity: 1
  })), index.c(`&.fade-in-height-expand-transition-${leaveClass}-to,
 &.fade-in-height-expand-transition-${enterClass}-from`, Object.assign(Object.assign({}, leaveToProps), {
  ...
  })), index.c(`&.fade-in-height-expand-transition-${leaveClass}-active`, `
 overflow: ${overflow};
 transition:
 ...
 `), index.c(`&.fade-in-height-expand-transition-${enterClass}-active`, `
 overflow: ${overflow};
 transition:
...
function self$2(vars) {
  ...
  return Object.assign(Object.assign({}, commonVariables$1), {
    ...
    boxShadowFocus: `0 0 0 2px ${index.changeColor(primaryColor, {
      alpha: 0.3
    })}`,
...
const checkboxGroupInjectionKey = index.createInjectionKey("n-checkbox-group");
...
const style$1 = index.c([
 ...
 `, [index.cM("show-label", "line-height: var(--n-label-line-height);"), index.c("&:hover", [index.cB("checkbox-box", [index.cE("border", "border: var(--n-border-checked);")])]), index.c("&:focus:not(:active)", [index.cB("checkbox-box", [index.cE("border", `
 border: var(--n-border-focus);
 box-shadow: var(--n-box-shadow-focus);
 `)])]), index.cM("inside-table", [index.cB("checkbox-box", `
 background-color: var(--n-merged-color-table);
 `)]), index.cM("checked", [index.cB("checkbox-box", `
 background-color: var(--n-color-checked);
 `, [index.cB("checkbox-icon", [
    // if not set width to 100%, safari & old chrome won't display the icon
    index.c(".check-icon", `
 opacity: 1;
 transform: scale(1);
 `)
  ])])]), index.cM("indeterminate", [index.cB("checkbox-box", [index.cB("checkbox-icon", [index.c(".check-icon", `
 opacity: 0;
 transform: scale(.5);
 `), index.c(".line-icon", `
 opacity: 1;
 transform: scale(1);
 `)])])]), index.cM("checked, indeterminate", [index.c("&:focus:not(:active)", [index.cB("checkbox-box", [index.cE("border", `
 border: var(--n-border-checked);
 box-shadow: var(--n-box-shadow-focus);
 `)])]), index.cB("checkbox-box", `
 background-color: var(--n-color-checked);
 border-left: 0;
 border-top: 0;
 `, [index.cE("border", {
 ...
 `, [index.cE("border", {
  ...
  }), index.cB("checkbox-icon", [index.c(".check-icon, .line-icon", {
 ...
 `, [index.c("&:empty", {
  ...
  index.insideModal(index.cB("checkbox", `
 --n-merged-color-table: var(--n-color-table-modal);
 `)),
  // popover table header checkbox
  index.insidePopover(index.cB("checkbox", `
 --n-merged-color-table: var(--n-color-table-popover);
 `))
]);
const checkboxProps = Object.assign(Object.assign({}, index.useTheme.props), {
  size: String,
  checked: {
...
const NCheckbox = vue.defineComponent({
  ...
  setup(props) {
    if (vue.process$1.env.NODE_ENV !== "production") {
      vue.watchEffect(() => {
        if (props.onChange) {
          index.warnOnce("checkbox", "`on-change` is deprecated, please use `on-update:checked` instead.");
        }
      });
    }
    ...
    const mergedCheckedRef = index.useMergedState(controlledCheckedRef, uncontrolledCheckedRef);
    const renderedCheckedRef = index.useMemo(() => {
      if (NCheckboxGroup) {
    ...
    const themeRef = index.useTheme("Checkbox", "-checkbox", style$1, checkboxLight, props, mergedClsPrefixRef);
    function toggle(e) {
      if (NCheckboxGroup && props.value !== void 0) {
        ...
        if (_onUpdateCheck) {
          index.call(_onUpdateCheck, nextChecked, e);
        }
        if (onUpdateChecked) {
          index.call(onUpdateChecked, nextChecked, e);
        }
        if (onChange) index.call(onChange, nextChecked, e);
        nTriggerFormInput();
        nTriggerFormChange();
    ...
    const rtlEnabledRef = index.useRtl("Checkbox", mergedRtlRef, mergedClsPrefixRef);
    const cssVarsRef = vue.computed(() => {
      ...
      const {
        ...
        self: {
          ...
          labelLineHeight,
          labelFontWeight,
          [index.createKey("fontSize", mergedSize)]: fontSize,
          [index.createKey("size", mergedSize)]: size
        }
      } = themeRef.value;
      return {
    ...
    const themeClassHandle = inlineThemeDisabled ? index.useThemeClass("checkbox", vue.computed(() => mergedSizeRef.value[0]), cssVarsRef, props) : void 0;
  ...
  render() {
    ...
    const labelNode = index.resolveWrappedSlot($slots.default, (children) => {
      if (label || children) {
        return vue.h("span", {
...
const style = index.c([index.cB("message-wrapper", `
 margin: var(--n-margin);
 z-index: 0;
 ...
 flex-shrink: 0;
 `, [["default", "info", "success", "warning", "error", "loading"].map((type) => index.cM(`${type}-type`, [index.c("> *", `
 ...
 right: 0;
 bottom: 0;
 `, [iconSwitchTransition()])]), index.cE("close", `
 ...
 `), index.cM("bottom-left", `
 bottom: 4px;
 left: 12px;
 right: 0;
 justify-content: flex-end;
 align-items: flex-start;
 `), index.cM("bottom-right", `
 bottom: 4px;
 left: 0;
 right: 12px;
 justify-content: flex-end;
...
const NMessage = vue.defineComponent({
  ...
  setup(props) {
    ...
    const {
      ...
      mergedClsPrefixRef
    } = vue.inject(index.messageProviderInjectionKey);
    const rtlEnabledRef = index.useRtl("Message", mergedRtlRef, mergedClsPrefixRef);
    const themeRef = index.useTheme("Message", "-message", style, messageLight, messageProviderProps2, mergedClsPrefixRef);
    const cssVarsRef = vue.computed(() => {
      const {
        type
      } = props;
      const {
        ...
        self: {
          ...
          closeIconSize,
          closeBorderRadius,
          [index.createKey("textColor", type)]: textColor,
          [index.createKey("boxShadow", type)]: boxShadow,
          [index.createKey("color", type)]: color,
          [index.createKey("closeColorHover", type)]: closeColorHover,
          [index.createKey("closeColorPressed", type)]: closeColorPressed,
          [index.createKey("closeIconColor", type)]: closeIconColor,
          [index.createKey("closeIconColorPressed", type)]: closeIconColorPressed,
          [index.createKey("closeIconColorHover", type)]: closeIconColorHover
        }
      } = themeRef.value;
      return {
        "--n-bezier": cubicBezierEaseInOut2,
    ...
    const themeClassHandle = inlineThemeDisabled ? index.useThemeClass("message", vue.computed(() => props.type[0]), cssVarsRef, {}) : void 0;
    return {
      mergedClsPrefix: mergedClsPrefixRef,
      rtlEnabled: rtlEnabledRef,
  ...
  render() {
    ...
    }, index.render(content)), closable ? vue.h(NBaseClose, {
...
const messageProviderProps = Object.assign(Object.assign({}, index.useTheme.props), {
  to: [String, Object],
  duration: {
    type: Number,
    default: 3e3
...
const NMessageProvider = vue.defineComponent({
  ...
  setup(props) {
    ...
    vue.provide(index.messageProviderInjectionKey, {
      props,
      mergedClsPrefixRef
    });
    vue.provide(index.messageApiInjectionKey, api);
    function create(content, options) {
      const key = index.createId();
...
const getSchemaIntrospection = async (options = {}) => {
  ...
  const resolvedExtensions = Array.isArray(extensions) && extensions.length ? extensions : mode === "docx" ? index.getStarterExtensions() : index.getRichTextExtensions();
  const tempEditor = new index.Editor({
...
class LocalStoragePolyfill {
  ...
  constructor(room) {
    this.room = room;
    this.onmessage = null;
    this._onChange = (e) => e.key === room && this.onmessage !== null && this.onmessage({ data: index.fromBase64(e.newValue || "") });
    index.onChange(this._onChange);
  }
  /**
   * @param {ArrayBuffer} buf
   */
  postMessage(buf) {
    index.varStorage.setItem(this.room, index.toBase64(index.createUint8ArrayFromArrayBuffer(buf)));
  }
  close() {
    index.offChange(this._onChange);
  }
}
const BC = typeof BroadcastChannel === "undefined" ? LocalStoragePolyfill : BroadcastChannel;
const getChannel = (room) => index.setIfUndefined(channels, room, () => {
  const subs = index.create$2();
...
const writeSyncStep1 = (encoder, doc) => {
  index.writeVarUint(encoder, messageYjsSyncStep1);
  const sv = Y__namespace.encodeStateVector(doc);
  index.writeVarUint8Array(encoder, sv);
};
const writeSyncStep2 = (encoder, doc, encodedStateVector) => {
  index.writeVarUint(encoder, messageYjsSyncStep2);
  index.writeVarUint8Array(encoder, Y__namespace.encodeStateAsUpdate(doc, encodedStateVector));
};
...
const writeUpdate = (encoder, update) => {
  index.writeVarUint(encoder, messageYjsUpdate);
  index.writeVarUint8Array(encoder, update);
};
...
const encodeQueryParams = (params) => index.map(params, (val, key) => `${encodeURIComponent(key)}=${encodeURIComponent(val)}`).join("&");
const messageSync = 0;
...
messageHandlers[messageSync] = (encoder, decoder, provider2, emitSynced, _messageType) => {
  index.writeVarUint(encoder, messageSync);
...
messageHandlers[messageQueryAwareness] = (encoder, _decoder, provider2, _emitSynced, _messageType) => {
  index.writeVarUint(encoder, messageAwareness);
  index.writeVarUint8Array(
    encoder,
...
const closeWebsocketConnection = (provider2, ws, event) => {
  if (ws === provider2.ws) {
    ...
    setTimeout(
      setupWS,
      index.min(
        index.pow(2, provider2.wsUnsuccessfulReconnects) * 100,
...
const setupWS = (provider2) => {
  if (provider2.shouldConnect && provider2.ws === null) {
    ...
    websocket.onmessage = (event) => {
      ...
      if (index.length(encoder) > 1) {
        websocket.send(index.toUint8Array(encoder));
      }
    };
    ...
    websocket.onopen = () => {
      ...
      if (provider2.awareness.getLocalState() !== null) {
        const encoderAwarenessState = index.createEncoder();
        index.writeVarUint(encoderAwarenessState, messageAwareness);
        index.writeVarUint8Array(
        ...
        websocket.send(index.toUint8Array(encoderAwarenessState));
      }
    };
    provider2.emit("status", [{
      status: "connecting"
...
class WebsocketProvider extends ObservableV2 {
  ...
  constructor(serverUrl, roomname, doc, {
    ...
    if (resyncInterval > 0) {
      ...
      setInterval(() => {
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
          ...
          this.ws.send(index.toUint8Array(encoder));
        }
      }, resyncInterval);
    }
    this._bcSubscriber = (data, origin) => {
      if (origin !== this) {
        ...
        if (index.length(encoder) > 1) {
          publish(this.bcChannel, index.toUint8Array(encoder), this);
        }
      }
    };
    this._updateHandler = (update, origin) => {
      if (origin !== this) {
        ...
        broadcastMessage(this, index.toUint8Array(encoder));
      }
    };
    ...
    if (index.isNode && typeof vue.process$1 !== "undefined") {
  ...
  connectBc() {
    ...
    const encoderSync = index.createEncoder();
    index.writeVarUint(encoderSync, messageSync);
    writeSyncStep1(encoderSync, this.doc);
    publish(this.bcChannel, index.toUint8Array(encoderSync), this);
    const encoderState = index.createEncoder();
    index.writeVarUint(encoderState, messageSync);
    writeSyncStep2(encoderState, this.doc);
    publish(this.bcChannel, index.toUint8Array(encoderState), this);
    const encoderAwarenessQuery = index.createEncoder();
    index.writeVarUint(encoderAwarenessQuery, messageQueryAwareness);
    publish(
      this.bcChannel,
      index.toUint8Array(encoderAwarenessQuery),
      this
    );
    const encoderAwarenessState = index.createEncoder();
    index.writeVarUint(encoderAwarenessState, messageAwareness);
    ...
    publish(
      this.bcChannel,
      index.toUint8Array(encoderAwarenessState),
      this
    );
  }
  disconnectBc() {
...
function createHocuspocusProvider({ config, user, documentId, socket, superdocInstance }) {
  ...
  provider$1.on("awarenessUpdate", (params) => {
...
function useSelection(params) {
  const documentId = vue.ref(params.documentId);
  const page = vue.ref(params.page);
  const selectionBounds = vue.reactive(params.selectionBounds || {});
  const source = vue.ref(params.source);
...
function useComment(params) {
  const uid = vue.ref(params.uid);
  const commentId = params.commentId || uuid.v4();
  ...
  const isInternal = vue.ref(params.isInternal !== void 0 ? params.isInternal : true);
  const mentions = vue.ref([]);
  const commentElement = vue.ref(null);
  const isFocused = vue.ref(params.isFocused || false);
  ...
  const importedAuthor = vue.ref(params.importedAuthor || null);
  ...
  const commentText = vue.ref(params.commentText || "");
  ...
  const trackedChange = vue.ref(params.trackedChange);
  const trackedChangeType = vue.ref(params.trackedChangeType || null);
  const trackedChangeText = vue.ref(params.trackedChangeText || null);
  const deletedText = vue.ref(params.deletedText || null);
  const resolvedTime = vue.ref(params.resolvedTime || null);
  const resolvedByEmail = vue.ref(params.resolvedByEmail || null);
  const resolvedByName = vue.ref(params.resolvedByName || null);
  const resolveComment = ({ email, name, superdoc }) => {
    ...
    if (trackedChange.value) {
      const emitData2 = { type: index.comments_module_events.RESOLVED, comment: getValues() };
      propagateUpdate(superdoc, emitData2);
      superdoc.activeEditor?.commands?.resolveComment({ commentId, importedId });
      return;
    }
    const emitData = { type: index.comments_module_events.RESOLVED, comment: getValues() };
  ...
  const setIsInternal = ({ isInternal: newIsInternal, superdoc }) => {
    ...
    const emitData = {
      type: index.comments_module_events.UPDATE,
...
function useDocument(params, superdocConfig) {
  ...
  const ydoc = vue.shallowRef(params.ydoc);
  const provider2 = vue.shallowRef(params.provider);
  const socket = vue.shallowRef(params.socket);
  const isNewFile = vue.ref(params.isNewFile);
  const editorRef = vue.shallowRef(null);
  const setEditor = (ref) => editorRef.value = ref;
  const getEditor = () => editorRef.value;
  const presentationEditorRef = vue.shallowRef(null);
  ...
  const rawFields = vue.ref(params.fields || []);
  const fields = vue.ref(params.fields?.map((f) => useField(f)) || []);
  const annotations = vue.ref(params.annotations || []);
  const conversations = vue.ref(initConversations());
  const conversationsBackup = vue.ref(conversations.value);
  const commentThreadingProfile = vue.ref(params.commentThreadingProfile || null);
  ...
  const removeConversation = (conversationId) => {
    const index2 = conversations.value.findIndex((c) => c.conversationId === conversationId);
...
const useSuperdocStore = /* @__PURE__ */ defineStore("superdoc", () => {
  ...
  const init = async (config) => {
    ...
    if (!configDocs?.length && !config.modules.collaboration) {
      const newDoc = await index.getFileObject(blankDocx.BlankDOCX, "blank.docx", DOCX);
      const newDocConfig = {
        type: DOCX,
        data: newDoc,
        name: "blank.docx",
  ...
  const _initializeDocumentData = async (doc) => {
    ...
    if (doc.data instanceof File) {
      ...
      try {
        const fileObject = await index.getFileObject(doc.url, doc.name || "document", doc.type);
        return { ...doc, data: fileObject };
      } catch (err) {
...
const useCommentsStore = /* @__PURE__ */ defineStore("comments", () => {
  ...
  const getGroupedComments = vue.computed(() => buildGroupedComments(compareByCreatedTime));
  const getCommentsByPosition = vue.computed(() => buildGroupedComments(compareByPosition));
  ...
  const deleteComment = ({ commentId: commentIdToDelete, superdoc }) => {
    ...
    const childCommentIds = commentsList.value.filter((c) => c.parentCommentId === commentId).map((c) => c.commentId || c.importedId);
  ...
  const createCommentForTrackChanges = (editor) => {
    ...
    groupedChanges.forEach(({ insertedMark, deletionMark, formatMark }, index$1) => {
      console.debug(`Create comment for track change: ${index$1}`);
      ...
      const isLastIteration = trackedChanges.length === index$1 + 1;
      if (foundComment) {
        if (isLastIteration) {
          tr.setMeta(index.CommentsPluginKey, { type: "force" });
        }
        return;
      }
      if (insertedMark || deletionMark || formatMark) {
        ...
        if (isLastIteration) tr.setMeta(index.CommentsPluginKey, { type: "force" });
        tr.setMeta(index.CommentsPluginKey, { type: "forceTrackChanges" });
        tr.setMeta(index.TrackChangesBasePluginKey, trackChangesPayload);
      }
      dispatch(tr);
    });
  };
  ...
  const convertHtmlToSchema = (commentHTML) => {
    const editor = new index.Editor({
      mode: "text",
      isHeadless: true,
      content: commentHTML,
      extensions: index.getRichTextExtensions()
    });
...
const superdocIcons = {
  comment: index.commentIconSvg,
  caretDown: index.caretDownIconSvg,
  internal: userCheckIconSvg,
  external: usersIconSvg,
  markDone: index.checkIconSvg,
  acceptChange: index.checkIconSvg,
  rejectChange: index.xmarkIconSvg,
  overflow: index.ellipsisVerticalSvg
};
...
const Avatar = /* @__PURE__ */ index._export_sfc(_sfc_main$g, [["__scopeId", "data-v-53e13009"]]);
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _cache) => {
      return vue.openBlock(), vue.createElementBlock("div", {
        ...
        vue.createVNode(vue.unref(index.NDropdown), {
          trigger: "click",
          options,
...
const InternalDropdown = /* @__PURE__ */ index._export_sfc(_sfc_main$f, [["__scopeId", "data-v-8b9e3b75"]]);
    ...
    return (_ctx, _cache) => {
      return vue.openBlock(), vue.createElementBlock("div", _hoisted_1$d, [
        ...
        vue.createBaseVNode("div", _hoisted_6$1, [
          ...
          allowOverflow.value ? (vue.openBlock(), vue.createBlock(vue.unref(index.NDropdown), {
            key: 2,
            trigger: "click",
...
const CommentHeader = /* @__PURE__ */ index._export_sfc(_sfc_main$e, [["__scopeId", "data-v-5e404f08"]]);
    ...
    return (_ctx, _cache) => {
      return vue.openBlock(), vue.createElementBlock("div", _hoisted_1$c, [
        ...
        vue.createBaseVNode("div", {
          ...
          vue.createVNode(vue.unref(index.SuperInput), {
...
const CommentInput = /* @__PURE__ */ index._export_sfc(_sfc_main$d, [["__scopeId", "data-v-5e1f953a"]]);
const _hoisted_1$b = {
  key: 0,
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _cache) => {
      ...
      return vue.withDirectives((vue.openBlock(), vue.createElementBlock("div", {
        ...
        shouldShowInternalExternal.value ? (vue.openBlock(), vue.createElementBlock("div", _hoisted_1$b, [
...
const CommentsList = /* @__PURE__ */ index._export_sfc(_sfc_main$b, [["__scopeId", "data-v-12d3e3dd"]]);
class SuperComments extends eventemitter3.EventEmitter {
  ...
  createVueApp() {
    ...
    this.app.directive("click-outside", index.vClickOutside);
...
const FloatingComments = /* @__PURE__ */ index._export_sfc(_sfc_main$9, [["__scopeId", "data-v-6868516c"]]);
const _sfc_main$8 = {
  __name: "TextField",
  props: {
    field: {
...
const TextField = /* @__PURE__ */ index._export_sfc(_sfc_main$8, [["__scopeId", "data-v-f3984d30"]]);
...
const ParagraphField = /* @__PURE__ */ index._export_sfc(_sfc_main$7, [["__scopeId", "data-v-ce5a87b3"]]);
...
const ImageField = /* @__PURE__ */ index._export_sfc(_sfc_main$6, [["__scopeId", "data-v-7dd69850"]]);
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return vue.openBlock(), vue.createElementBlock("div", _hoisted_1$5, [
        ...
        }, vue.toDisplayString(getValue.value ? "x" : ""), 5))
      ]);
    };
  }
};
const CheckboxField = /* @__PURE__ */ index._export_sfc(_sfc_main$5, [["__scopeId", "data-v-a135a0a5"]]);
...
const SelectField = /* @__PURE__ */ index._export_sfc(_sfc_main$4, [["__scopeId", "data-v-1e49d9b3"]]);
...
const HrbrFieldsLayer = /* @__PURE__ */ index._export_sfc(_sfc_main$3, [["__scopeId", "data-v-22e6ae02"]]);
...
const HtmlViewer = /* @__PURE__ */ index._export_sfc(_sfc_main$2, [["__scopeId", "data-v-da3494ba"]]);
const _hoisted_1$1 = {
...
const AiLayer = /* @__PURE__ */ index._export_sfc(_sfc_main$1, [["__scopeId", "data-v-497b8118"]]);
...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    const PdfViewer = vue.defineAsyncComponent(() => Promise.resolve().then(() => require("./PdfViewer-B6ZnU2PL.cjs")));
    const superdocStore = useSuperdocStore();
    const commentsStore = useCommentsStore();
    const {
      documents,
    ...
    const onEditorSelectionChange = ({ editor, transaction }) => {
      ...
      const presentation = index.PresentationEditor.getInstance(documentId);
      if (!presentation) {
    ...
    const editorOptions = (doc) => {
      ...
      const options = {
        ...
        editorCtor: useLayoutEngine ? index.PresentationEditor : void 0,
    ...
    vue.watch(
      ...
      (zoom) => {
        if (proxy.$superdoc.config.useLayoutEngine !== false) {
          index.PresentationEditor.setGlobalZoom((zoom ?? 100) / 100);
        }
      }
    );
    ...
    return (_ctx, _cache) => {
      const _directive_click_outside = vue.resolveDirective("click-outside");
      return vue.openBlock(), vue.createBlock(vue.unref(index.NConfigProvider), {
        abstract: "",
        "preflight-style-disabled": ""
      }, {
        default: vue.withCtx(() => [
          vue.createBaseVNode("div", {
            class: vue.normalizeClass(["superdoc", {
              "superdoc--with-sidebar": showCommentsSidebar.value,
              "superdoc--web-layout": vue.unref(proxy).$superdoc.config.viewOptions?.layout === "web",
                ...
                (vue.openBlock(true), vue.createElementBlock(vue.Fragment, null, vue.renderList(vue.unref(documents), (doc) => {
                  return vue.openBlock(), vue.createElementBlock("div", {
                    ...
                    vue.createVNode(vue.unref(NMessageProvider), null, {
                      default: vue.withCtx(() => [
                        doc.type === vue.unref(DOCX) ? (vue.openBlock(), vue.createBlock(vue.unref(index.SuperEditor), {
                          key: 0,
            ...
            vue.unref(showAiWriter) ? (vue.openBlock(), vue.createElementBlock("div", {
              ...
              vue.createVNode(vue.unref(index.AIWriter), {
...
const App = /* @__PURE__ */ index._export_sfc(_sfc_main, [["__scopeId", "data-v-ba1f348b"]]);
const createSuperdocVueApp = () => {
  const app = vue.createApp(App);
  const pinia = createPinia();
  app.use(pinia);
  app.directive("click-outside", index.vClickOutside);
  const superdocStore = useSuperdocStore();
  const commentsStore = useCommentsStore();
...
class SuperDoc extends eventemitter3.EventEmitter {
  ...
  #addToolbar() {
    ...
    this.toolbar = new index.SuperToolbar(config);
  ...
  async export({
    ...
    const zip = await superEditor_fileZipper.createZip(blobsToZip, filenames);
    if (triggerDownload) {
...
exports.DOCX = DOCX;
exports.HTML = HTML;
exports.NBaseLoading = NBaseLoading;
exports.PDF = PDF;
exports.SuperDoc = SuperDoc;
exports.compareVersions = compareVersions;
exports.getSchemaIntrospection = getSchemaIntrospection;
exports.storeToRefs = storeToRefs;
exports.useSelection = useSelection;
exports.useSuperdocStore = useSuperdocStore;
